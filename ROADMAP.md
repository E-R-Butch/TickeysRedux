...
- **开机自启** ✓ — 使用 macOS 13+ 的 `SMAppService`，处理启用、待批准、失败和注销状态
- **偏好设置窗口** ✓ — 音效、通用、关于三个标签页
- **音量持久化迁移** ✓ — 统一为 0–100 百分比，修复重启后几乎静音
- **睡眠与监听恢复** ✓ — 正确确认 IOKit 休眠事件，并重启 Event Tap 而不是重启进程
- **隐藏图标后的入口** ✓ — 再次打开 App 会显示偏好设置
- **发布校验** ✓ — DMG 布局、ZIP、版本、架构、部署目标和 SHA-256 均自动验证
...
基础 NSWindow 设置面板已经实现；v1.1 继续补完首次启动向导和高级控制。菜单栏保留方案快速切换，"Preferences…" 打开面板。

面板内容：

- **音量 / 音调** ✓ — NSSlider 滑杆已实现；可编辑输入框和点位吸附待补
- **开机自启** ✓ — 通过 `SMAppService.mainAppService`，不手写 LaunchAgent
