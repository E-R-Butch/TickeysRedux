...
pub fn show_prefs_window(_mtm: MainThreadMarker, tickeys_ptr: *mut Tickeys) {
    unsafe {
        ...
        if !PREFS_WINDOW.is_null() {
            let _: () = msg_send![PREFS_WINDOW, makeKeyAndOrderFront: std::ptr::null::<AnyObject>()];
            return;
        }
        autoreleasepool(|_pool| { build_and_show_window(); });
    }
}
...
define_class!(
    ...
    impl SettingsController {
        ...
        fn toggle_start_at_login(&self, sender: &AnyObject) {
            ...
            let _ = crate::launcher::set_start_at_login(state == 1);
        }
...
unsafe fn build_sound_view(ctrl: &SettingsController) -> *mut AnyObject {
    ...
    unsafe {
        ...
        add_label(container, &nsstring_to_string(&l10n_str("sound_scheme")), 30.0, y, 100.0, 20.0);
        let popup = make_popup(140.0, y - 4.0, 240.0, &schemes, &pref_scheme, ctrl);
        let _: () = msg_send![container, addSubview: popup];
        y -= 40.0;

        add_label(container, &nsstring_to_string(&l10n_str("volume")), 30.0, y, 80.0, 20.0);
        ...
        let vol_label = add_label(container, &format!("{}", pref_vol as i32), 330.0, y - 6.0, 50.0, 20.0);
        VOL_LABEL = vol_label;
        y -= 40.0;

        add_label(container, &nsstring_to_string(&l10n_str("pitch")), 30.0, y, 80.0, 20.0);
        let slider = make_slider(100.0, y - 4.0, 220.0, 0.1, 2.0, pref_pitch);
        set_target(slider, ctrl, sel!(changePitch:));
        let _: () = msg_send![container, addSubview: slider];
        let pitch_label = add_label(container, &format!("{:.1}x", pref_pitch), 330.0, y - 6.0, 50.0, 20.0);
...
unsafe fn build_general_view(ctrl: &SettingsController) -> *mut AnyObject {
    let start_at_login = crate::launcher::is_start_at_login_enabled();
    ...
    unsafe {
        ...
        let btn = make_checkbox(&nsstring_to_string(&l10n_str("start_at_login")), 30.0, y, start_at_login);
        set_target(btn, ctrl, sel!(toggleStartAtLogin:));
        let _: () = msg_send![container, addSubview: btn];
        y -= 35.0;

        let btn = make_checkbox(&nsstring_to_string(&l10n_str("show_in_menu_bar")), 30.0, y, show_in_menu_bar);
...
unsafe fn build_about_view() -> *mut AnyObject {
    unsafe {
        ...
        // Version
        let ver = add_label(container, &format!("v1.0.5  •  macOS 15+  •  arm64"), 80.0, y, 280.0, 18.0);
        let _: () = msg_send![ver, setFont: small_font];
        let _: () = msg_send![ver, setAlignment: 1u32];
        y -= 30.0;

        // Description
        let desc = add_label(container, "机械键盘音效模拟工具 — 每次击键都带来真实的打字体验。", 30.0, y, 380.0, 34.0);
        ...
        let a1 = add_label(container, "应元东 — github.com/yingDev/Tickeys", 120.0, y, 290.0, 18.0);
        ...
        let a2 = add_label(container, "Sinclair Liu — github.com/E-R-Butch/TickeysRedux", 120.0, y, 290.0, 18.0);
        ...
        // Built with
        let bw = add_label(container, "Built with Rust • objc2 • rodio • 致敬 yingDev 的 Tickeys", 30.0, y, 380.0, 16.0);
...
unsafe fn add_label(parent: *mut AnyObject, text: &str, x: f64, y: f64, w: f64, h: f64) -> *mut AnyObject {
    unsafe {
        let tf: *mut AnyObject = msg_send![class("NSTextField"), alloc];
        let _: () = msg_send![tf, initWithFrame: NSRect::new(NSPoint::new(x, y), NSSize::new(w, h))];
...
unsafe fn make_popup(x: f64, y: f64, w: f64, schemes: &[AudioScheme], current: &str, ctrl: &SettingsController) -> *mut AnyObject {
    unsafe {
        ...
        for (i, scheme) in schemes.iter().enumerate() {
            ...
            if scheme.name == current { let _: () = msg_send![pb, selectItemAtIndex: i as isize]; }
        }
...
unsafe fn make_slider(x: f64, y: f64, w: f64, min: f64, max: f64, value: f32) -> *mut AnyObject {
    unsafe {
        ...
        let _: () = msg_send![slider, initWithFrame: NSRect::new(NSPoint::new(x, y), NSSize::new(w, 24.0))];
...
unsafe fn set_target(control: *mut AnyObject, ctrl: &SettingsController, action: objc2::runtime::Sel) {
    unsafe {
...
fn load_pref_string(key: &str) -> Option<String> {
    unsafe {
        ...
        if val.is_null() { None } else {
            Some(CStr::from_ptr(msg_send![val, UTF8String]).to_string_lossy().to_string())
        }
    }
}
...
fn load_pref_float(key: &str) -> f32 {
    unsafe {
        ...
        if val.is_null() {
            match key { "audio_volume" => 50.0, "audio_pitch" => 1.0, _ => 0.0 }
        } else {
